...
| S21: check the overpasses; between Mexikói út and Kőbánya-Kispest the train runs well above the road | 26 Sep | open. Checked 26 Sep: the rail is never more than 3 m above the ground 70 m either side anywhere from Nyugati to km 11.5; the Kőbánya yard stands on a terrace above the road beside it. **Need the exact spot** (a screenshot) |
| Üllői út crossing (S21) does not work for cars: in reality there is no level difference there | 26 Sep | open. Found 26 Sep: OSM tags the primary road at S21 km 11.5 (just past KÖKI) `bridge=yes` within 10–50 m of the line, so the game lifts it 8.2 m over the rails. If that is the crossing meant, OSM or the rail-bridge rule is wrong there; **confirm the place** |
...
| Kispest: the owner sent photos of how it really looks | 26 Sep | open (**the images did not arrive**; ask again) |
